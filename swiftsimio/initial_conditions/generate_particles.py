...
class ParticleGenerator(object):
    ...
    def initial_setup(
        ...
        if x is None:
            if method == "rejection":
                ...
                self.coordinates = self.IC_perturbed_coordinates()
            elif method == "uniform":
        ...
        #  self.rho = np.zeros(self.npart, dtype=np.float)
        #  self.h = np.zeros(self.npart, dtype=np.float)
    ...
    def redistribute_particles(
        self, rhoA: np.ndarray, iteration: int,
    ):
        """
        ...
        to_move = int(npart * self.runparams.redist_frac)
