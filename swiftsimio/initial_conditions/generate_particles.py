...
class ParticleGenerator(object):
    ...
    # unyt/result arrays
    coordinates: Union[unyt_array, None] = None
    masses: Union[unyt_array, None] = None
    smoothing_lengths: Union[unyt_array, None] = None
    densities: Union[unyt_array, None] = None
    ...
    def initial_setup(
        ...
        if x is None:
            ...
            if not isinstance(x, unyt_array):
    ...
    def perform_iteration(
        ...
        for d in range(ndim):
            dev += delta_r[:, d] ** 2
        dev = np.sqrt(dev)
        dev /= ipars.mean_interparticle_distance
        ...
        stop = False
        if (
            max_deviation < runparams.displ_thresh
        ):  # don't think about stopping until max < threshold
            unconverged = dev[dev > runparams.converge_thresh].shape[0]
