"""
Runs tests accessing data via hdfstream server
...
test_data_path = "SWIFT/test_data/IOExamples/ssio_ci_04_2025"

# Override the test fixtures that would normally download the snapshots
@pytest.fixture(
    params=[
        {"filename" : f"{test_data_path}/EagleDistributed.hdf5", "server" : server},
        {"filename" : f"{test_data_path}/EagleSingle.hdf5", "server" : server},
        {"filename" : f"{test_data_path}/LegacyCosmologicalVolume.hdf5", "server" : server},
    ]
)
